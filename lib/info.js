...
const infoEncoding = {
  preencode (state, info) {
    cenc.fixed64.preencode(state, info.secretKey)
    cenc.fixed32.encode(state, info.publicKey)
    cenc.buffer.encode(state, info.signature)
    cenc.uint.encode(state, info.fork)
    return state
  },
  encode (state, info) {
    cenc.fixed64.encode(state, info.secretKey || Buffer.alloc(0))
    cenc.fixed32.encode(state, info.publicKey)
    cenc.buffer.encode(state, info.signature)
    cenc.uint.encode(state, info.fork)
    return state
