...
module.exports = class Info {
  ...
  async open (opts) {
    await new Promise((resolve) => {
      this.storage.read(0, 64 + 32 + 64 + 8, (_, buf) => {
        if (buf) {
          const sigStart = 64 + 32 + 8
          const sigEnd = sigStart + uint64le.decode(sigLen, 64 + 32)

          this.secretKey = notZero(buf.slice(0, 64))
          this.publicKey = buf.slice(64, 64 + 32)
          this.signature = notZero(buf.slice(sigStart, sigEnd))
          this.fork = uint64le.decode(buf, sigEnd)
        }
        resolve()
      })
    })
  ...
  commit () {
    const sigLen = this.signature ? this.signature.byteLength : 0
    const buf = Buffer.alloc(64 + 32 + 8 + sigLen + 8)
    if (this.secretKey !== null) this.secretKey.copy(buf)
    this.publicKey.copy(buf, 64)
    uint64le.encode(sigLen, 64 + 32)
    if (this.signature) this.signature.copy(buf, 64 + 32)
    uint64le.encode(this.fork, buf, 64 + 32 + 8 + sigLen)
    return buf
  }
