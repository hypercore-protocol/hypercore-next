...
wire.info = {
  ...
  encode (state, m) {
    ...
    c.uint.encode(state, (m.uploading ? 1 : 0) | (m.downloading ? 2 : 0))
  },
  decode (state) {
    const core = c.uint.decode(state)
    const flags = c.uint.decode(state)

    return {
      core,
      uploading: (flags & 1) !== 0,
      downloading: (flags & 2) !== 0
    }
  }
}
...
wire.unwant = {
  preencode (state, m) {
    c.uint.preencode(state, m.core)
    c.uint.preencode(state, m.want)
  },
  encode (state, m) {
    c.uint.encode(state, m.core)
    c.uint.encode(state, m.want)
  },
  decode (state, m) {
    return {
      core: c.uint.decode(state),
      want: c.uint.decode(state)
    }
  }
}
...
wire.upgrade = {
  preencode (state, m) {
    ...
    c.uint.preencode(state, m.fork)
    c.uint.preencode(state, m.length)
    c.uint.preencode(state, m.upgradeableLength)
  },
  encode (state, m) {
    c.uint.encode(state, m.core)
    c.uint.encode(state, m.fork)
    c.uint.encode(state, m.length)
    c.uint.encode(state, m.upgradeableLength)
  },
  decode (state) {
    return {
      core: c.uint.decode(state),
      fork: c.uint.decode(state),
      length: c.uint.decode(state),
      upgradeableLength: c.uint.decode(state)
    }
  }
}
