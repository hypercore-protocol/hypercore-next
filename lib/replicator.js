...
class RequestPool {
  ...
  updateUpgrade (peer) {
    if (this.upgrading || peer.state.length < this.pendingUpgrade.minLength) return false
  ...
  async _onfork (proof) {
    ...
    const lca = this.unforking || this.core.tree.lca()
    const done = await lca.verify(proof)

    if (!this.unforking) {
      const verified = this.core.verifySignature(lca.signable(), lca.signature)
      if (!verified) throw new Error('Remote signature could not be verified')
      this.unforking = lca
    }

    if (!done) return console.log('TODO: fork not done, impl me')

    await this.core.verifyFork(this.unforking.end, this.unforking.proof())
    this.unforking = null
  ...
  async ondata (proof, peer) {
    ...
    if (peer.state.fork !== this.core.fork) {
      ...
      if (peer.state.fork > this.core.fork) return this._onfork(proof)
      return
    }
...
module.exports = class Replicator {
  ...
  createStream () {
    ...
    this.core.opening.then(() => {
      ...
      if (this.core.length > 0) peer.info({ length: this.core.length, fork: this.core.fork })
  ...
  oninfo ({ length, fork }, peer) {
    ...
    if (forked) {
      ...
      if (fork > this.core.fork) peer.request({ fork })
    }
  ...
  onoptions ({ extensions }, peer) {
    console.log('in onoptions:', extensions)
    peer.state.remoteExtensions.update(extensions)
  }

  onextension ({ id, data }, peer) {
    console.log('in onextension:', id, data)
    peer.state.remoteExtensions.onmessage(id, data, this)
  }
