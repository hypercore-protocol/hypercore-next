...
class RequestPool {
  ...
  _updateRange (peer, range) {
    ...
    if (range.linear) return !!this._requestRange(peer, range._start, end, 0, range.filter)

    const r = range._start + Math.floor(Math.random() * (end - range._start))
    return !!(
      this._requestRange(peer, r, end, 0, range.filter) ||
      this._requestRange(peer, range._start, r, 0, range.filter)
    )
  }
  ...
  _requestRange (peer, start, end, seek, filter = tautology) {
    const remote = peer.state.bitfield
    const local = this.core.bitfield
