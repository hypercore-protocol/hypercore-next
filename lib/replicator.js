...
// Saving the first cap to be used for signatures
const [, INITIATOR, RESPONDER] = crypto.namespace('hypercore', 3)
...
class Peer {
  constructor (replicator, protomux) {
    this.core = replicator.core
    this.replicator = replicator
    this.stream = protomux.stream
    this.protomux = protomux

    this.session = protomux.open({
      context: this,
      protocol: 'hypercore/alpha',
      id: replicator.discoveryKey,
      messages: [
        { encoding: m.wire.handshake, onmessage: onwirehandshake },
    ...
    this.wireHandshake = this.session.messages[0]
    this.wireSync = this.session.messages[1]
    this.wireRequest = this.session.messages[2]
    this.wireCancel = null
    this.wireData = this.session.messages[4]
    this.wireNoData = this.session.messages[5]
    this.wireWant = this.session.messages[6]
    this.wireUnwant = this.session.messages[7]
    this.wireBitfield = this.session.messages[8]
    this.wireRange = this.session.messages[9]
    this.wireExtension = this.session.messages[10]
  ...
  onhandshake ({ capability }) {
    const expected = cap(this.stream, this.replicator.key, true)
...
module.exports = class Replicator {
  ...
  addUpgrade (session) {
    if (this._upgrade !== null) return this._upgrade.attach(session)
  ...
  _makePeer (protomux) {
    ...
    const peer = new Peer(this, protomux)

    peer.wireHandshake.send({
      capability: cap(protomux.stream, this.key, false)
    })
...
function cap (stream, key, inverse) {
  const capability = b4a.allocUnsafe(32)
  const ns = stream.isInitiator !== inverse ? INITIATOR : RESPONDER

  sodium.crypto_generichash_batch(capability, [stream.handshakeHash, key], ns)
...
function onwirehandshake (m, s) {
  return s.context.onhandshake(m)
}

function onwiresync (m, s) {
  return s.context.onsync(m)
}

function onwirerequest (m, s) {
  return s.context.onrequest(m)
}

function onwiredata (m, s) {
  return s.context.ondata(m)
}

function onwirenodata (m, s) {
  return s.context.onnodata(m)
}

function onwirewant (m, s) {
  return s.context.onwant(m)
}

function onwireunwant (m, s) {
  return s.context.onunwant(m)
}

function onwirebitfield (m, s) {
  return s.context.onbitfield(m)
}

function onwirerange (m, s) {
  return s.context.onrange(m)
}

function onwireclose (isRemote, s) {
  return s.context.onclose(isRemote)
}

function onwireextension (m, s) {
  return s.context.onextension(m)
}
