...
class WantRequest extends Attachable {
  constructor (wants, start, length) {
    super()
  ...
  _unref () {
    for (const peer of this.peers) peer.wants.remove(this)
    this.peers = []

    const i = this.wants.indexOf(this)
    if (i === -1) return
    const h = this.wants.pop()
    if (i < this.wants.length - 1) this.wants[i] = h
  }
...
class Peer {
  ...
  onopen ({ capability }) {
    ...
    this.sendSync()
    this.broadcastRange(0, this.core.header.contiguousLength, false)
  ...
  _requestWant (w) {
    if (this.wants.has(w)) return false
    this.wants.add(w)
    ...
    length = Math.min(length === -1 ? this.core.tree.length : length, this.core.tree.length)

    this.wireWant.send({ start, length })
...
module.exports = class Replicator {
  ...
  addWant (session, { index, start = index, end = index + 1, length = toLength(start, end) }) {
    // TODO: caching and merging of wants
    const r = new WantRequest(this._wants, start, length)
    const ref = r.attach(session)
  ...
  _onwant (peer, start, length) {
    ...
    // TODO: we can probably do better than run-length chunks here
    for (let i = 0; i < length; i++) {
      if (this.core.bitfield.get(start + i) === false) continue

      let end = i

      while (end < length && this.core.bitfield.get(start + end) === true) end++
  ...
  _updatePeer (peer) {
    ...
    for (const w of this._wants) {
      if (peer._requestWant(w) === true) {
        return true
      }
    }
