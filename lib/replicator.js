...
class Request {
  ...
  resolve (val) {
    for (let i = 0; i < this.deferreds.length; i++) {
      this.deferreds[i].resolve(val)
    }
  }

  reject (err) {
    for (let i = 0; i < this.deferreds.length; i++) {
      this.deferreds[i].reject(err)
    }
  }
}
...
class UpgradeLock extends Deferred {
...
class Range {
  ...
  _done (err, done) {
    ...
    if (err) this._deferred.reject(err)
    else this._deferred.resolve(done)
  }
...
class RequestPool {
  ...
  async send (peer, req) {
    ...
    if (fork !== this.core.tree.fork || paused(peer, this.core.tree.fork) || this.core.truncating > 0) {
      ...
      if (req.deferreds.length) { // someone is eagerly waiting for this request
