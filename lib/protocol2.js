...
module.exports = class HypercoreProtocol {
  constructor (mux, onunknowncore) {
    this.muxer = Protomux.from(mux)
    this.stream = this.muxer.stream

    this.onunknowncore = onunknowncore || noop
    this.closed = false
    ...
    this.info = this.protocol.addMessage({ type: 2, encoding: m.wire.info, onmessage: oninfo })
    this.upgrade = this.protocol.addMessage({ type: 3, encoding: m.wire.upgrade, onmessage: onupgrade })
    this.request = this.protocol.addMessage({ type: 4, encoding: m.wire.request, onmessage: onrequest })
    this.cancel = this.protocol.addMessage({ type: 5, encoding: m.wire.cancel, onmessage: oncancel })
    this.data = this.protocol.addMessage({ type: 6, encoding: m.wire.data, onmessage: ondata })
    this.noData = this.protocol.addMessage({ type: 7, encoding: m.wire.noData, onmessage: onnodata })
    this.want = this.protocol.addMessage({ type: 8, encoding: m.wire.want, onmessage: onwant })
    this.unwant = this.protocol.addMessage({ type: 9, encoding: m.wire.unwant, onmessage: onunwant })
    this.bitfield = this.protocol.addMessage({ type: 10, encoding: m.wire.bitfield, onmessage: onbitfield })
    this.range = this.protocol.addMessage({ type: 11, encoding: m.wire.range, onmessage: onrange })
    this.reorgHint = this.protocol.addMessage({ type: 12, encoding: m.wire.reorgHint, onmessage: onreorghint })
  }
...
async function onopen (msg, protocol) {
  ...
  if (!map) {
    ...
    try {
      await protocol.onunknowncore(msg.discoveryKey)
    } finally {
...
async function oninfo (msg, protocol) {
  const peer = protocol._opens.length > 0 ? await protocol._async(msg.core) : protocol._sync(msg.core)
  if (peer !== null) return peer.oninfo(msg)
}

async function onupgrade (msg, protocol) {
  const peer = protocol._opens.length > 0 ? await protocol._async(msg.core) : protocol._sync(msg.core)
  if (peer !== null) return peer.onupgrade(msg)
}
