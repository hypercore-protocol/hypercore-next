...
class MerkleTreeBatch {
  ...
  snapshot () {
    ...
    batch.tree = this.tree
    batch.nodes = [...this.nodes]
    return batch
  }
  ...
  rewind (length) {
    ...
    this.nodes = this.nodes.slice(0, sliceTo)

    this.length = length
    this.byteLength = this.nodes.reduce((acc, n) => acc + n.size, 0)
    this.roots = this.nodes.filter(n => roots.includes(n.index))

    if (head === 0) return this

    const ite = flat.iterator(head)
    this.appendRoot(this.nodes.pop(), ite)
...
module.exports = class MerkleTree extends Flushable {
  constructor (storage, roots, fork) {
    super()
  ...
  static async open (storage, opts = {}) {
    ...
    return new MerkleTree(storage, roots, opts.fork || 0)
  }
}
