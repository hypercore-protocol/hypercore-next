...
class MerkleTreeBatch {
  constructor (tree) {
    ...
    this.signable = tree.logic.signable.bind(this)
    this.signedBy = tree.logic.signedBy.bind(this)
  ...
  signable () {
    return this.tree.logic.signable().bind(this)
  }

  signedBy (verificationInfo) {
    return this.tree.logic.signedBy(this.signable(), this.signature, verificationInfo)
  }
  ...
  commit () {
    if (!this.upgraded) this.treeLength = this.tree.length

    if (this.treeLength !== this.tree.length || this.treeFork !== this.tree.fork) {
...
module.exports = class MerkleTree extends Flushable {
  constructor (storage, roots, fork, opts = {}) {
    super()

    this.crypto = opts.crypto || defaultCrypto
    this.logic = opts.logic ? opts.logic : HypercoreLogic
    this.fork = fork
    this.roots = roots
    this.length = roots.length ? totalSpan(roots) / 2 : 0
    this.byteLength = totalSize(roots)

    this.signable = this.logic.signable.bind(this)
    this.signedBy = this.logic.signedBy.bind(this)
  ...
  signable () {
    return this.logic.signable().bind(this)
  }

  signedBy (verificationInfo) {
    return this.logic.signedBy(this.signable(), this.signature, verificationInfo)
  }
  ...
  _flushTruncation () {
    return new Promise((resolve, reject) => {
      ...
      const offset = t === 0 ? 0 : (t - 1) * 80 + 32 + 40
  ...
  async truncate (length, opts = { fork: this.fork }) {
    const head = length * 2
  ...
  async proof ({ block, seek, upgrade }, signature = null) {
  ...
  static async open (storage, opts = {}) {
    ...
    return new MerkleTree(storage, roots, opts.fork || 0, opts)
  }
}
...
function getStoredNode (storage, index, error) {
  return new Promise((resolve, reject) => {
    storage.read(32 + 40 * index, 40, (err, data) => {
      if (err) {
...
function storedNodes (storage) {
  return new Promise((resolve) => {
    storage.stat((_, st) => {
      ...
      resolve(((st.size - 32) - ((st.size - 32) % 40)) / 40)
    })
  })
}
...
function truncateMap (map, len) {
  for (const node of map.values()) {
    if (node.index >= len) map.delete(node.index)
  }
}
