...
class ByteSeeker {
  ...
  async _seek (bytes) {
    ...
    for (const node of this.tree.roots) { // all async ticks happen once we find the root so safe
      let size = node.size
      if (this.padding > 0) size -= this.padding * countLeaves(node.index)
      ...
      while ((ite.index & 1) !== 0) {
        ...
        if (l) {
          let size = l.size
          if (this.padding > 0) size -= this.padding * countLeaves(l.index)
...
function countLeaves (index) {
  return twoPow(flat.depth(index))
}
