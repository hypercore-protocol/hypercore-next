...
module.exports = class Core {
  ...
  static createAuth (crypto, { publicKey, secretKey }) {
    if (secretKey && !crypto.validateKeyPair({ publicKey, secretKey })) throw new Error('Invalid key pair')

    const sign = secretKey ? (signable) => crypto.sign(signable, secretKey) : undefined
  ...
  async truncate (length, fork, sign = this.defaultAuth.sign) {
    this.truncating++
    await this._mutex.lock()
  ...
  async append (values, sign = this.defaultAuth.sign, hooks = {}) {
    await this._mutex.lock()
