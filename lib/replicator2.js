...
class Peer {
  ...
  onremoteopen ({ capability }) {
    const expected = cap(this.protocol.stream, this.replicator.key, true)

    if (b4a.equals(capability, expected) !== true) {
      throw new Error('Invalid capability received')
    }
  ...
  _remoteCanUpgrade (oldLen) {
    return oldLen < this.remoteLength && this.lengthAcked === this.core.tree.length && this.remoteCanUpgrade === true
  }

  _upgradeAvailable (u) {
    return u.fork === this.core.tree.fork && this._remoteCanUpgrade(u.length)
  }
...
module.exports = class Replicator {
  ...
  addRange (session, { start = 0, length = -1, blocks = null, linear = false }) {
    if (blocks !== null) {
    ...
    const r = new RangeRequest(this._ranges, this.core.tree.fork, start, end, linear, blocks)
  ...
  _checkUpgradeIfAvailable () {
    if (this._peersPending > 0 || this._upgrade === null) return
  ...
  attachTo (protocol) {
    ...
    protocol.stream.opened.then((open) => {
      if (open === false) return

      protocol.open.send({
        id: peer.alias,
        discoveryKey: this.discoveryKey,
        capability: cap(protocol.stream, this.key, false)
      })
    })
  }
}
